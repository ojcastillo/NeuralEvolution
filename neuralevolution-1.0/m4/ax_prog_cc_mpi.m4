...
AS_IF([test x"$_ax_prog_cc_mpi_mpi_wanted" = xno],
  ...
  [
    ...
    AS_IF([test x"$_ax_prog_cc_mpi_mpi_found" = xyes], [
      save_CFLAGS=$CFLAGS
	  CFLAGS="-I/usr/include/mpich2 $save_CFLAGS"
...
AC_DEFUN([_AX_PROG_CC_MPI], [
  ...
  if test x"$_ax_prog_cc_mpi_mpi_wanted" = xyes; then
	if test -n "$MPICC"; then
      ...
      AC_CHECK_TOOLS([CC], [mpicc.mpich2 mpicc mpixlc_r mpixlc hcc mpxlc_r mpxlc sxmpicc mpifcc mpgcc mpcc cmpicc cc gcc])
    fi
  fi
])dnl _AX_PROG_CC_MPI
